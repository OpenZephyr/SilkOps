...
Result: one JSON report from `silkops milestone-sync --issues <set.json>`:
...
2. Write `{"plan": "adhoc-<basename>", "milestone": "<title>", "issues": [{"unit":
   ...
   summary, location, fix, `Source: <path>`. Run `silkops milestone-sync --project P --issues <f>
   --run <id>`. The milestone must already exist.
3. Proof, when the operator names the fix: `silkops note --project P --issue <iid> --body-file
